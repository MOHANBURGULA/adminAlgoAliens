...
export type EvaluationDisplayStatus = "passed" | "failed" | "pending"
...
const EVALUATION_STATUS_META: Record<EvaluationDisplayStatus, StatusMeta> = {
  passed: {
    label: "Passed",
    color: "#22c55e",
    badgeClassName: "bg-emerald-500/15 text-emerald-200 border border-emerald-500/20",
  ...
  failed: {
    label: "Failed",
    color: "#f87171",
    badgeClassName: "bg-rose-500/15 text-rose-200 border border-rose-500/20",
  },
  pending: {
    label: "Pending",
    color: "#fbbf24",
...
export function normalizeEvaluationStatus(status: string) {
...
export function getEvaluationStatusMeta(status: string | EvaluationDisplayStatus) {
  return EVALUATION_STATUS_META[normalizeEvaluationStatus(status)]
}
...
export function buildEvaluationTableRows(
  ...
  return [...evaluations]
    ...
    .map((evaluation) => ({
      ...
      userName: userMap.get(evaluation.userId) ?? `User #${evaluation.userId}`,
      courseName: courseMap.get(evaluation.courseId) ?? `Course #${evaluation.courseId}`,
      score:
        typeof evaluation.finalScore === "number" && Number.isFinite(evaluation.finalScore)
          ? Math.round(evaluation.finalScore)
          : null,
      status: normalizeEvaluationStatus(evaluation.status),
      rawStatus: evaluation.status,
      aiDetectionScore:
        typeof evaluation.aiDetectionScore === "number" && Number.isFinite(evaluation.aiDetectionScore)
          ? Math.round(evaluation.aiDetectionScore)
          : null,
      createdAt: formatAdminDate(evaluation.createdAt),
    })) satisfies EvaluationTableRow[]
}

export function buildEvaluationStatusChartData(evaluations: AdminEvaluationRecord[]) {
  const counts = {
    passed: 0,
    failed: 0,
    pending: 0,
  ...
  for (const evaluation of evaluations) {
    counts[normalizeEvaluationStatus(evaluation.status)] += 1
  }

  return [
    {
      name: EVALUATION_STATUS_META.passed.label,
      value: counts.passed,
      color: EVALUATION_STATUS_META.passed.color,
    },
    {
      name: EVALUATION_STATUS_META.failed.label,
      value: counts.failed,
      color: EVALUATION_STATUS_META.failed.color,
    },
    {
      name: EVALUATION_STATUS_META.pending.label,
      value: counts.pending,
      color: EVALUATION_STATUS_META.pending.color,
    },
