...
export default function EnrollmentsAdminPage() {
  ...
  useEffect(() => {
    ...
    const loadEnrollments = async () => {
      try {
        ...
        if (!cancelled) {
          setError(getApiErrorMessage(loadError, "Unable to load enrollments."))
        }
      } finally {
        if (!cancelled) {
          setLoading(false)
  ...
  }, [courseFilter, debouncedSearch, progressFilter, rows])
  ...
  const filteredAverageProgress = useMemo(
    () => calculateAverageProgress(filteredRows),
    [filteredRows],
  )
  ...
  if (loading) {
    return <div className="text-gray-300">Loading enrollments...</div>
  }
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-gray-400">
          Review learner progress with responsive filters, summary insights, and a mobile-friendly
          progress table.
        </p>
      </div>
      ...
      <FilterBar
        summary={`${filteredRows.length} visible | ${filteredAverageProgress}% average progress in view | ${filteredCompletedUsers} completed in view`}
      >
        <input
          value={search}
