...
// Change #3 #5 — video now has feedback, rejectionCount, permanently_rejected status
...
// Change #9 #15 — course now includes description, keywords, categoryId
...
// Change #11 — new CourseCategory type
...
// Change #10 — analytics grouped by category
export type AdminCategoryAnalytics = {
  categoryId: string
  categoryName: string
  courseCount: number
  enrollmentCount: number
...
// Change #3 #5 — video status now includes permanently_rejected
export function getVideoStatusMeta(status: string): StatusMeta {
  ...
  if (normalized === "permanently_rejected") {
    return { label: "Permanently Rejected", tone: "slate" }
  }
