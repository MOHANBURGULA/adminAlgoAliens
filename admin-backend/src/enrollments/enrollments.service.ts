...
// UPDATED — Change #2: CertificatesService removed from updateProgress.
// Certificates are now ONLY issued when admin approves a video via
// PUT /api/admin/videos/:id/status { status: 'approved' }
@Injectable()
export class EnrollmentsService {
  ...
  async getByUser(userId: number) {
    ...
    if (cachedEnrollments === null) {
      await this.redisService.setCache(cacheKey, enrollments, CACHE_TTL_SECONDS.enrollmentsUser)
    }
  ...
  // Change #2 — certificate issuance removed from here.
  // Progress tracking still works; certificate only comes from admin video approval.
  ...
  private async ensureUserCoursesCache(userId: number, enrollments: Enrollment[]) {
    ...
    await this.redisService.setCache(userCoursesCacheKey, userCourses, CACHE_TTL_SECONDS.coursesUser)
    return userCourses
  }
}
