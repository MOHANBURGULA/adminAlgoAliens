...
export class AuthService {
  ...
  async login(data: LoginPayload) {
    ...
    if (this.isConfiguredAdminCredentials(normalizedData.email, normalizedData.password)) {
      ...
      this.logger.debug(
        `Login success email=${adminUser.email} role=${adminUser.role} tokenIssued=true`,
      )
    ...
    // CHANGE #1 — single generic error to prevent user enumeration
    if (!existingUser) {
    ...
    if (existingUser.password === 'GOOGLE_AUTH') {
      this.logger.warn(
        `Login failed: Google account attempted password login for ${existingUser.email}`,
      )
      await this.redisMetricsService.increment(MetricNames.authLoginFailure)
    ...
    // CHANGE #1 — same generic message for wrong password (no 'Invalid password' separately)
    if (!valid) {
