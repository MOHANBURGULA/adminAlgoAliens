...
import { VideoProcessingJobData } from '../evaluation/video-processing.types'

// Change #2 — CertificatesService import REMOVED.
// Certificates are now only issued when an admin explicitly approves
// a video via PUT /api/admin/videos/:id/status { status: 'approved' }
// See admin.service.ts → updateVideoStatus()
...
export class VideoProcessingProcessor {
  ...
  async process(job: VideoProcessingJobData) {
    ...
    try {
      ...
      // Change #2 — NO automatic certificate issuance here.
      // Admin must approve the video; that triggers certificate issuance.
